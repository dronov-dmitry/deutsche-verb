...
// SQLite database lifecycle: schema, seeding, migrations, queries.
// Supports mobile (sqflite), desktop (sqflite_common_ffi), and web (sqflite_common_ffi_web).
...
import '_database_factory.dart' show initDatabaseFactory;
...
class DatabaseService {
  ...
  Future<void> init() async {
    ...
    print('[DB] Pre-computing asset hashes...');
    final buffer = StringBuffer();
    for (final asset in _seededAssets) {
      final start = DateTime.now();
      final content = await rootBundle.loadString(asset);
      final hash = _djb2(content).toRadixString(16);
      buffer.write(hash);
      buffer.write(':');
      print('[DB] Hashed $asset in ${DateTime.now().difference(start).inMilliseconds}ms');
    }
    ...
    _db = await openDatabase(
      ...
      version: 12,
      onCreate: (db, _) async {
        print('[DB] onCreate: Database creation started...');
        
        print('[DB] onCreate: Seeding schema.sql...');
        final schema = await rootBundle.loadString('assets/database/schema.sql');
        await db.transaction((txn) async {
          await _executeSqlFile(txn, schema);
        });

        print('[DB] onCreate: Seeding data.sql...');
        ...
        print('[DB] onCreate: Seeding descriptions.sql...');
        ...
        print('[DB] onCreate: Seeding example_translations_uk.sql...');
        ...
        print('[DB] onCreate: Seeding links.sql...');
        final links = await rootBundle.loadString('assets/database/links.sql');
        await db.transaction((txn) async {
          await _executeSqlFile(txn, links, originalPrefix: null);
        });

        print('[DB] onCreate: Seeding descriptions_verbformen.sql (large)...');
        final startVf = DateTime.now();
        final vfDesc = await rootBundle.loadString('assets/database/descriptions_verbformen.sql');
        print('[DB] onCreate: Loaded descriptions_verbformen.sql in ${DateTime.now().difference(startVf).inMilliseconds}ms');
        
        final startTransform = DateTime.now();
        ...
        print('[DB] onCreate: Transformed descriptions_verbformen.sql in ${DateTime.now().difference(startTransform).inMilliseconds}ms');
            
        final startExec = DateTime.now();
        await db.transaction((txn) async {
          await _executeSqlFile(txn, vfDescTransformed, originalPrefix: null);
        });
        print('[DB] onCreate: Executed descriptions_verbformen.sql in ${DateTime.now().difference(startExec).inMilliseconds}ms');

        // Seed the hash to prevent _checkAndReseedIfNeeded running immediately on fresh install
        await db.insert('meta', {'key': 'asset_hash', 'value': currentHash});
        print('[DB] onCreate: Database creation finished successfully!');
      },
    ...
    await _checkAndReseedIfNeeded(_db!, currentHash);
  }

  /// Compares the pre-computed assets hash with the value stored in the `meta`
  /// table. Re-seeds if they differ.
  Future<void> _checkAndReseedIfNeeded(Database db, String currentHash) async {
